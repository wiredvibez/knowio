...
export default function NetworkPage() {
  ...
  const [importResult, setImportResult] = useState<null | { created: number; updated: number; skipped: number; errors: number }>(null);
  ...
  async function ensureTags(category: 'from' | 'relationship' | 'character' | 'field', names: string[], uid: string): Promise<string[]> {
    ...
    for (const name of names) {
      ...
      if (!trimmed) continue;
      const normalizedId = trimmed.toLowerCase().replace(/\s+/g, "_");
        ...
        for (let start = 0; start < dataRows.length; start += chunkSize) {
          ...
          for (const cols of chunk) {
            try {
              ...
              // Compute tag diffs for update flow
              let add: string[] = [], remove: string[] = [];
              if (exists) {
                const prev = prevData as any;
                const d1 = computeTagDiffs(prev?.from, fromIds);
                const d2 = computeTagDiffs(prev?.relationship, relationshipIds);
                const d3 = computeTagDiffs(prev?.character, characterIds);
                const d4 = computeTagDiffs(prev?.field, fieldIds);
                add = [...d1.add, ...d2.add, ...d3.add, ...d4.add];
                remove = [...d1.remove, ...d2.remove, ...d3.remove, ...d4.remove];
              } else {
                add = [...fromIds, ...relationshipIds, ...characterIds, ...fieldIds];
              }
