...
export function TagPicker({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <div className="fixed inset-x-4 top-20 z-50 mx-auto max-w-2xl rounded-xl bg-background p-4 shadow-lg">
        <div className="flex items-center gap-2">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="חיפוש תגיות" />
          {mode === "edit" && (
            <Button onClick={() => q && createTag(q)} variant="secondary">+ צור "{q}"</Button>
          )}
